// --------------------------------------------------------------------------------
// ASCOM Camera driver for cam86 v.0.9.7
// Edit Log:
...
// 21/1/2023    Tommy Ramberg    0.9.7   DLL ported to C++ with updated calling convetions and timer. Version number updated to reflect compatibility with the DLL driver.
//                                       
// --------------------------------------------------------------------------------

/*  Copyright © 2017 Gilmanov Rim, Vakulenko Sergiy, Luka Pravica and Tommy Ramberg.
...
namespace ASCOM.cam86
{
    ...
    public class Camera : ICameraV2
    {
        ...
        /// </summary>
        internal static string driverVersion = "0.9.7";
        ...
        // Changed Ansi to Unicode
        // Changed StdCall to Cdecl
        // Removed "CallingConvention = CallingConvention.Cdecl, " before CharSet.
        // Imports cam86ll.dll functions
        [DllImport(LowLevelDLL, CharSet = CharSet.Unicode)]
        ...
        [DllImport(LowLevelDLL, CharSet = CharSet.Unicode)]
        static extern uint cameraGetImage(); // This is not good. It really returns a pointer to a int.
        [DllImport(LowLevelDLL, CharSet = CharSet.Unicode)]
        ...
        public object ImageArray
        {
            get
            {
                ...
                uint imagepoint; // This is not good. It really returns a pointer to a int.
                //Get image pointer
                tl.LogMessage("ImageArray Get", "Call cameraGetImage from " + LowLevelDLL);
                imagepoint = cameraGetImage();
                
                /*
                // This converts an IntPtr to a 1 dimension Int32 array and then to a 2 dimensional Int32 array.
                IntPtr cameraImagePtr = (IntPtr)cameraGetImage();
                Int32[] cameraImageArray1 = new Int32[cameraNumX*cameraNumY];
                Marshal.Copy(cameraImagePtr, cameraImageArray1, 0, (cameraNumX*cameraNumY));
                Int32[,] cameraImageArray2 = new Int32[cameraNumX, cameraNumY];
                for(int i = 0; i< cameraNumX; i++)
                {
                    for (int j = 0; j<cameraNumY;j++)
                    {
                        cameraImageArray2[i, j] = cameraImageArray1[(i * cameraNumY) + j];
                    }
                }
                */
                ...
                unsafe
                {
                    // All this does it find the min and max values of the image array and fill "cameraImageArray"
                    // Is it possible to return the whole image array from the dll?
